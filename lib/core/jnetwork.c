...
j_network_connection_init_client(JConfiguration* configuration, JBackendType backend, guint index, JNetworkConnection** instance_ptr)
{
	...
	*instance_ptr = g_new0(JNetworkConnection, 1);
	...
	CHECK_GERROR(error, end, "Failed to read addr format from g_connection!");
	...
	CHECK_GERROR(error, end, "Failed to read addr len from g_connection!");
	...
	CHECK_GERROR(error, end, "Failed to read addr from g_connection!");
	...
	EXE(j_network_fabric_init_client(configuration, &jf_addr, &connection->fabric), end, "Failed to initialize fabric for client!");
