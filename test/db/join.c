...
perform_join_on_table1_table2(void)
{
	...
	g_assert_no_error(error);
	success = j_db_selector_add_field(selector_tableA, "orderid", J_DB_SELECTOR_OPERATOR_GE, &customerid, sizeof(customerid), &error);
	g_assert_true(success);
	g_assert_no_error(error);
	selector_tableB = j_db_selector_new(schema_tableB, J_DB_SELECTOR_MODE_AND, &error);
	g_assert_nonnull(selector_tableB);
	g_assert_no_error(error);
	success = j_db_selector_add_field(selector_tableB, "customerid", J_DB_SELECTOR_OPERATOR_GE, &customerid, sizeof(customerid), &error);
	g_assert_true(success);
	g_assert_no_error(error);
	...
	while (j_db_iterator_next(iterator, NULL))
	{
		...
		printf("DB Field: orderid: %llu, orderdate: %s, firstname: %s.\n", *orderid, orderdate, firstname);
...
void
test_join_queries(void)
{
	// FIXME add more tests
	g_test_add_func("/join/all", test_join_all);
}
