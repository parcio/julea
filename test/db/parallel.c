...
test_parallel_insert_select_setup(void)
{
	...
	thread_pool = g_thread_pool_new(test_parallel_insert_select, schema, j_configuration_get_max_connections(j_configuration()), TRUE, NULL);
...
{
	g_test_add_func("/db/parallel/insert_select", test_parallel_insert_select_setup);
}
