...
int
config_get_version(struct Config* conf)
{
	return conf->version;
}
struct fi_info*
config_get_hints(struct Config* conf)
{
	return conf->hints;
}
void
config_set_max_op_size(struct Config* conf, size_t size)
{
	conf->max_op_size = size;
}
size_t
config_get_max_op_size(struct Config* conf)
{
	return conf->max_op_size;
}
struct Config*
config_init(void)
{
...
gboolean
fabric_init_client(struct Config* config, struct fi_info* hints, struct jfabric** fabric_ptr)
{
	int res;
	struct jfabric* fabric;
	const char* location;
...
gboolean
fabric_init_server(struct Config* config, struct jfabric** fabric_ptr)
{
	int res;
	struct jfabric* fabric;
	const char* location;
...
gboolean
fabric_fini(struct jfabric* fabric)
{
	int res;
...
gboolean
read_pep_eq(struct jfabric* fabric, enum Event* event, struct fi_eq_cm_entry* entry)
{
	int res;
	const char* location;
	uint32_t fi_event;
...
gboolean
open_socket(int* res, uint32_t port)
{
	int fd;
	int error;
	...
	if (fd == -1)
	{
		g_error("failed to open listening socket! error: %s", strerror(errno));
		goto end;
	}
	error = setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (const char*)&one, sizeof(one));
	if (error == -1)
	{
		g_error("failed to set socket option! error: %s", strerror(errno));
		goto end;
	}
	...
	if (error == -1)
	{
		g_error("filed to set address for socket (binding)! error: %s", strerror(errno));
		goto end;
	}
...
gboolean
socket_wait_for_connection(int fd, int* active_socket)
{
	int error;
	int a_fd;
	...
	if (error == -1)
	{
		g_error("failed to listen for connections! error: %s", strerror(errno));
		goto end;
	}
	...
	if (a_fd == -1)
	{
		g_error("failed to accept connection request! error: %s", strerror(errno));
		goto end;
	}
...
gboolean
socket_write_addr(int fd, struct jfabric* fabric)
{
	int res;
	size_t addrlen = 0;
	const char* location;
	...
	if ((res != -FI_ETOOSMALL) || addrlen <= 0)
	{
		CHECK("failed to fetch address size!");
	}
	addr = malloc(addrlen);
	...
	// DEBUG:
	{
		...
		for (i = 0; i < 16; ++i)
		{
			snprintf(str + i * 3, 4, "%02x ", ((uint8_t*)addr)[i]);
		}
	...
	if (res == -1 || (size_t)res != sizeof(len))
	{
		g_error("failed to send addrlen!(%i)", res);
		goto end;
	}
	res = send(fd, &fabric->info->addr_format, sizeof(fabric->info->addr_format), 0);
	if (res == -1 || (size_t)res != sizeof(fabric->info->addr_format))
	{
		g_error("failed to send addr format!(%i)", res);
		goto end;
	}
	res = send(fd, addr, addrlen, 0);
	if (res == -1 || (size_t)res != addrlen)
	{
		g_error("failed to send addr!(%i)", res);
		goto end;
	}
...
gboolean
parse_address(char* addr, uint32_t port, struct addrinfo** infos)
{
	const char* err_msg;
	char port_s[6];
	int res;
...
gboolean
socket_request_addr(char* s_addr, uint32_t s_port, struct fi_info* hints)
{
	int res;
	struct addrinfo* itr;
	int fd;
	...
	for (itr = addresses; itr; itr = itr->ai_next)
	{
		...
		if (fd == -1)
		{
			g_error("failed to open connection: error: %s", strerror(errno));
			continue;
		}
	...
	if (!itr)
	{
		g_error("Unable to connect to any host!");
		goto end;
	}
	freeaddrinfo(addresses);

	res = recv(fd, (void*)&addr_len, sizeof(addr_len), 0);
	if (res == -1 || res != sizeof(addr_len))
	{
		g_error("failed to recive address len!");
		goto end;
	}
	addr_len = ntohl(addr_len);
	res = recv(fd, (void*)&addr_format, sizeof(addr_format), 0);
	if (res == -1 || res != sizeof(addr_format))
	{
		g_error("failed to recive address format!");
		goto end;
	}
	addr = malloc(addr_len);
	res = recv(fd, addr, addr_len, 0);
	if (res == -1 || (uint32_t)res != addr_len)
	{
		g_error("failed to recive address!");
		goto end;
	}
...
gboolean
endpoint_fini(struct jendpoint* endpoint)
{
	int res;
...
gboolean
_endpoint_create_mr(struct jendpoint* endpoint)
{
	int res;
	gboolean tx_prefix;
	gboolean rx_prefix;
...
gboolean
endpoint_create(struct jendpoint* endpoint)
{
	int res;
	const char* location;
	...
	res = fi_cq_open(endpoint->domain, &(struct fi_cq_attr){ .wait_obj = FI_WAIT_UNSPEC, .format = FI_CQ_FORMAT_CONTEXT, .size = endpoint->info->tx_attr->size },
			 &endpoint->txcq, &endpoint->rxcq);
	CHECK("failed to create txcq!");
	res = fi_cq_open(endpoint->domain, &(struct fi_cq_attr){ .wait_obj = FI_WAIT_UNSPEC, .format = FI_CQ_FORMAT_CONTEXT, .size = endpoint->info->rx_attr->size },
...
gboolean
endpoint_init_client(struct jendpoint** endpoint_ptr, struct Config* config, char* addr, uint32_t port)
{
	struct fi_info* hints;
...
gboolean
endpoint_init_server(struct jendpoint** endpoint_ptr, struct jfabric* fabric, struct fi_eq_cm_entry* con_req)
{
	int res;
	const char* location;
	...
	*endpoint_ptr = malloc(sizeof(struct jendpoint));
	...
	{
		g_error("failed to ack connection!");
		goto end;
	}
	...
	*endpoint_ptr = NULL;
	...
	if (res < 0)
	{
		g_error("%s: failed to reject connection:\n\tDetails: %s", location, fi_strerror(-res));
	}
	return FALSE;
}
gboolean
endpoint_read_eq(struct jendpoint* endpoint, enum Event* evt, struct fi_eq_cm_entry* entry)
{
	int res;
	const char* location;
	uint32_t fi_event;
	...
	{
		case FI_CONNREQ:
			g_error("endpoint can't  get connection request!");
			goto end;
		case FI_CONNECTED:
			*evt = CONNECTED;
			break;
...
gboolean
endpoint_check_connection(struct jendpoint* endpoint)
{
	return endpoint->ep != NULL && !endpoint->shutdown;
}

gboolean
endpoint_wait_for_completion(struct jendpoint* endpoint, enum ConnectionDirection con_dir, int len, gboolean* check, void** contexts)
{
	int i, count = 0;
	struct fid_cq* cq;
...
gboolean
ssend_message(struct jendpoint* endpoint, struct message* msg)
{
	int res;
	const char* location = endpoint->fabric->location;
	...
	if (ack != ACK)
	{
		g_error("recived wrong ack!: %i, expected: %i", ack, ACK);
		goto end;
	}
...
gboolean
srecv_message(struct jendpoint* endpoint, struct message** msg_ptr)
{
	int res;
	const char* location = endpoint->fabric->location;
...
struct message*
construct_message(void)
{
	struct message* msg = malloc(sizeof(struct message));

	msg->len = 2;
	msg->data = malloc(sizeof(*msg->data) * msg->len);
	msg->data[0].buff_size = 6;
	msg->data[0].buff_addr = (uint64_t)g_strdup(buffers[0]);
	msg->data[1].buff_size = 7;
	msg->data[1].buff_addr = (uint64_t)g_strdup(buffers[1]);
	return msg;
}
void
free_message(struct message* msg, gboolean sender)
{
	size_t i;
	if (msg == NULL)
	{
	...
	if (msg->data != NULL)
	{
		for (i = 0; i < msg->len; ++i)
		{
			free((void*)msg->data[i].buff_addr);
		}
		free(msg->data);
	}
	free(msg);
...
void
print_message(struct message* msg)
{
	int i;
...
gboolean
start_new_thread(struct jfabric* fabric, struct fi_eq_cm_entry* con_req, GArray* threads)
{
...
void
client(char* addr, uint32_t port)
{
	struct jendpoint* endpoint;
